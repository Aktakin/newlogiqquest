...
/**
 * The browser animates a run locally, but only this endpoint can award stars.
 * Both sides execute the same engine, so the outcome always matches.
 */
app.post('/api/players/:playerId/attempts', (req: Request, res: Response) => {
  ...
  const level = typeof gameId === 'string' && typeof levelId === 'string'
...
/**
 * Case Cipher rounds are priced here rather than in the browser. The client
 * only reports which prompt it asked and what was typed; the answer itself
 * never leaves the server's copy of the bank.
 */
